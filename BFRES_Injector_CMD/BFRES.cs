    ...
    class BFRES
    {
        ...
        public class MeshObj
        {
            public void InjectMesh(Model input, Syroot.BinaryData.ByteOrder BO)
            {
                ...
                input.Shapes[ShapeIndex].Meshes[0].SetIndices(theFaces);
                if (input.Shapes[ShapeIndex].Meshes.Count != 1) for (int i = 0; i < input.Shapes[ShapeIndex].Meshes.Count; i++) input.Shapes[ShapeIndex].Meshes.RemoveAt(1);
                input.Shapes[ShapeIndex].Radius = 10000000000f;

                Console.WriteLine("Inecting FMDL " + FMDLIndex + "Injecting Shaoe " + ShapeIndex);

                input.Shapes[ShapeIndex].Meshes[0].SubMeshes.Clear();
                ...
                for (int i = 0; i < input.Shapes[ShapeIndex].SubMeshBoundings.Count; i++) input.Shapes[ShapeIndex].SubMeshBoundings[i] = LeeT;

                SubMesh Setup = new SubMesh();
                Setup.Count = (uint)meshes[0].faces.Count;
                Setup.Offset = 0;
                for (int vvv = 0; vvv < 1; vvv++) input.Shapes[ShapeIndex].Meshes[0].SubMeshes.Add(Setup);
